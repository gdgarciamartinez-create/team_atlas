...
def run_scalping_m5(symbol: str, tf: str, candles: List[Dict[str, Any]], digits: int):
    """
    Versión M5 (misma idea que M1, pero con threshold un poco más amplio).
    Si querés, después lo afinamos. Ahora lo dejamos sólido y simple.
    """
    if len(candles) < 30:
        return {
            "symbol": symbol,
    ...
    impulse = lc - pc
    threshold = abs(pc) * 0.00035  # M5: umbral un poco más exigente
...
def build_snapshot(
    ...
    """
    Snapshot estándar para ATLAS_IA (SCALPING_M5).
    """
    ...
    result = run_scalping_m5(symbol, tf, candles or [], digits)
